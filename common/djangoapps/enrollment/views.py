...
class EnrollmentListView(APIView, ApiKeyPermissionMixIn):
    ...
    def post(self, request):
        ...
        if username != request.user.username and not (has_api_key_permissions or request.user.is_superuser):
            # Return a 404 instead of a 403 (Unauthorized). If one user is looking up
            # other users, do not let them deduce the existence of an enrollment.
            return Response(status=status.HTTP_404_NOT_FOUND)

        if mode not in (CourseMode.AUDIT, CourseMode.HONOR, None) and not (has_api_key_permissions or request.user.is_superuser):
            return Response(
