...
class VerticalGrade(CourseGrade):
    @staticmethod
    def grade(student, request, course, keep_raw_scores, field_data_cache, scores_client):
        ...
        - section_breakdown : A breakdown of each item that makes
        ...
        for item_format, items in grading_context['graded_items'].iteritems():
            format_scores = []
            for item in items:
                item_descriptor = item['item_descriptor']
                item_name = item_descriptor.display_name_with_default

                # some problems have state that is updated independently of interaction
                # with the LMS, so they need to always be scored. (E.g. foldit.,
                # combinedopenended)
                should_grade_section = any(
                    descriptor.always_recalculate_grades for descriptor in item['xmoduledescriptors']
                )

                # If there are no problems that always have to be regraded, check to
                # see if any of our locations are in the scores from the submissions
                # API. If scores exist, we have to calculate grades for this item.
                if not should_grade_section:
                    should_grade_section = any(
                        descriptor.location.to_deprecated_string() in submissions_scores
                        for descriptor in item['xmoduledescriptors']
                    )
                ...
                # If we haven't seen a single problem in the item, we don't have
                ...
                if should_grade_section:
                    ...
                    descendants = yield_dynamic_descriptor_descendants(item_descriptor, student.id, create_module)
                    ...
                    __, graded_total = graders.aggregate_scores(scores, item_name)
                    ...
                    graded_total = Score(0.0, 1.0, True, item_name, None)
                ...
                if graded_total.possible > 0:
                    ...
                    log.info(
                        "Unable to grade a item with a total possible score of zero. " +
                        str(item_descriptor.location)
                    )

            totaled_scores[item_format] = format_scores
    ...
    def grading_context(course):
        ...
        graded_items - This contains the sections that are graded, as
            ...
            arrays of dictionaries containing
                "item_descriptor" : The section descriptor
        ...
        for chapter in course.get_children():
            for section in chapter.get_children():
                if section.graded:
                    ...
                    graded_items[section_format] = graded_items.get(section_format, []) + [item_description]

                    all_descriptors.extend(xmoduledescriptors)
                    all_descriptors.append(section)

        return {'graded_items': graded_items,
