...
def access_token_registration(olga_acceptor_url):
    """
    Registry edX installation on OLGA acceptor.

    Acceptor returns access token for further API flow: authorization and dispatch statistics.
    """
...
def get_acceptor_api_access_token(olga_acceptor_url):
    ...
    If access token does not exist, method goes to registry it.
